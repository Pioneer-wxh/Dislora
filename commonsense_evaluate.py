...
def main(
    ...
    def evaluate(
        ...
        outputs = [extract_response(o) for o in outputs]
        print(outputs)
        return outputs
    ...
    for idx, batch in enumerate(batches):
        ...
        inputs = [data.get('input') for data in batch] if any('input' in data for data in batch) else None
...
def extract_answer(args, sentence: str) -> str:
    ...
    if dataset == 'boolq':
        pred_answers = re.findall(r'true|false', sentence, re.IGNORECASE)
        if not pred_answers:
            return ""
        return pred_answers[0].lower()
    elif dataset == 'piqa':
        pred_answers = re.findall(r'solution1|solution2', sentence, re.IGNORECASE)
        if not pred_answers:
            return ""
        return pred_answers[0].lower()
    elif dataset in ['social_i_qa', 'ARC-Challenge', 'ARC-Easy', 'openbookqa']:
        pred_answers = re.findall(r'answer1|answer2|answer3|answer4|answer5', sentence, re.IGNORECASE)
        if not pred_answers:
            return ""
        return pred_answers[0].lower()
    elif dataset == 'hellaswag':
        pred_answers = re.findall(r'ending1|ending2|ending3|ending4', sentence, re.IGNORECASE)
        if not pred_answers:
            return ""
        return pred_answers[0].lower()
    elif dataset == 'winogrande':
        pred_answers = re.findall(r'option1|option2', sentence, re.IGNORECASE)
        if not pred_answers:
            return ""
        return pred_answers[0].lower()
    return ""
