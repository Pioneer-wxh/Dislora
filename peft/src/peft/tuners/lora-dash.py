    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.lora_w_u =  nn.Parameter(self.weight.new_zeros((out_features, self.index)))
            self.lora_w_vt = nn.Parameter(self.weight.new_zeros((self.index, in_features)))
            self.warmup = 100
            self.FLAG = 0
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0:
                self.weight.data += (
                    ...
                    self.lora_w_u @ torch.diag(self.lora_index.weight.view(-1)) @ self.lora_w_vt
                )
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.FLAG < self.warmup:
                ...
                weight_u, weight_sigma, weight_vt = torch.linalg.svd(self.weight.to(dtype=torch.float32), full_matrices=False)
                delta_weight_sigma = torch.diag(torch.matmul(torch.matmul(weight_u.T, delta_W), weight_vt.T))
                top_index = self.calculate_change_rate(weight_sigma, delta_weight_sigma, self.index)

                self.lora_w_u = nn.Parameter(weight_u[:, top_index].to(self.lora_A.weight.dtype))
                self.lora_w_vt = nn.Parameter(weight_vt[top_index, :].to(self.lora_A.weight.dtype))
                self.lora_w_u.requires_grad = False
                self.lora_w_vt.requires_grad = False
            ...
            if self.FLAG > self.warmup:
                result += self.lora_dropout(x) @ (self.lora_w_u @ torch.diag(self.lora_index.weight.view(-1)) @ self.lora_w_vt).T
                return result
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
