...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.lora_E = nn.Linear(r, r, bias=False)
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A"):
            ...
            nn.init.zeros_(self.lora_E.weight)

    def train(self, mode: bool = True):
        nn.Linear.train(self, mode)
        self.lora_A.train(mode)
        self.lora_E.train(mode)
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0:
                self.weight.data += (
                    transpose(self.lora_B.weight @ self.lora_E.weight @ self.lora_A.weight, self.fan_in_fan_out) * self.scaling
                )
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                result += ((self.lora_dropout(x.to(self.lora_A.weight.dtype)) @ self.lora_A.weight.T) @ self.lora_E.weight.T @ self.lora_B.weight.T) * self.scaling
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
