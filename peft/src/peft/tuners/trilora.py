...
class Linear(nn.Linear, LoraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if r > 0:
            ...
            self.lora_E = nn.Linear(1, r, bias=False)
    ...
    def train(self, mode: bool = True):
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0:
                self.weight.data += (
                    transpose(self.lora_B.weight @ (self.lora_A.weight * self.lora_E.weight), self.fan_in_fan_out) * self.scaling
                )
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            ...
            if self.r > 0:
                result += ((self.lora_dropout(x.to(self.lora_A.weight.dtype)) @ (self.lora_A.weight * \
                 self.lora_E.weight).T) @ self.lora_B.weight.T) * self.scaling
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
